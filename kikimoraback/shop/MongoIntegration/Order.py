...
class Order:
    ...
    def insert_insales_number(self, payment_id: str, insales_order_number: int)->bool:
        ...
        """
        try:
            updated_order = self.order_collection.find_one_and_update({"payment_id": payment_id},
                                                                      {"$set": {'insales': insales_order_number,
                                                                                'status': 'PAYED'}})
            ...
            logger.error("По какой-то причине произошла ошибка при присвоении заказа номера из insales.\n"
