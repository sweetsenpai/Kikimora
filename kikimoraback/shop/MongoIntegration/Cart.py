...
class Cart:
    ...
    def create_cart(self, user_id, front_cart_data):
        if not self.get_cart_data(user_id):
            self.cart_collection.insert_one(
                {
                    "customer": user_id,
    ...
    def sync_cart_data(self, user_id, front_cart_data):
        ...
        try:
            if front_cart_data:
                ...
                if back_user_data:
                    logger.info(
                        f"Данные для корзины пользователя с id {user_id} загружены из mongo_db."
                    )
                    return self.get_cart_data(user_id)["products"]
        except Exception as e:
            logger.error(
                f"Произошла ошибка при синхронизации корзины. Данные корзины frontend: {front_cart_data}\n"
        ...
        if delivery_data["deliveryMethod"] == "Доставка":
                ...
                {"customer": user_id},
                {
                    "$set": {
                        ...
                        "delivery_data.cost": (
                            ...
                            if delivery_data["time"] == "custom"
                            else delivery_data["deliveryCost"]
                        ),
            ...
            self.cart_collection.update_one(
                {"customer": user_id},
                {
                    ...
                    "$inc": {"total": 1000 if delivery_data["time"] == "custom" else 0},
                },
            )
            return
    ...
    def remove_payement_id(self, payment_id, user_id):
        self.cart_collection.update_one(
            {"customer": user_id}, {"$unset": {"payment_id": payment_id}}
        )
